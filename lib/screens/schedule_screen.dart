...
class _ScheduleScreenState extends State<ScheduleScreen> {
  ...
  // Schedule log
  final List<_ScheduleLog> _scheduleLogs = [];
  ...
  void _addLog(String type, String msg) {
    ...
    debugPrint('[$t] $msg');
    setState(() => _scheduleLogs.insert(0, _ScheduleLog(type: type, time: t, msg: msg)));
  ...
  Future<void> _saveConfig() async {
    ...
    _addLog('success', 'Config saved successfully');
  ...
  Future<void> _testConnection() async {
    ...
    _addLog('info', 'Testing connection: ${_userCtrl.text.trim()}@${_hostCtrl.text.trim()}:${_portCtrl.text}');
    try {
      await _api.sftpPost('sftp/test', _sftpBody);
      setState(() { _connected = true; _connecting = false; });
      _addLog('success', 'SFTP connected successfully');
    } catch (e) {
  ...
  Future<void> _runNow() async {
    ...
    _addLog('info', '=== Manual Run Started ===');
    _addLog('info', 'Date folder: $dateFolder');
    _addLog('info', 'Path: ${_pathCtrl.text.trim()}/$dateFolder/emboss');

    try {
      // Get files
      _addLog('info', 'Loading file list...');
      final filesRes = await _api.sftpPost('sftp/files', {..._sftpBody, 'dateFolder': dateFolder});
      final files    = List<String>.from(filesRes['files'] ?? []);

      if (files.isEmpty) {
        _addLog('warn', 'No files found for today ($dateFolder)');
        setState(() { _running = false; _lastRun = _formatDateTime(today); });
        return;
      }

      _addLog('success', 'Found ${files.length} file(s): ${files.join(", ")}');
      int totalCards = 0;

      for (final fileName in files) {
        _addLog('info', 'Processing: $fileName');
        try {
          final res   = await _api.sftpPost('sftp/import', {..._sftpBody, 'dateFolder': dateFolder, 'fileName': fileName});
          final count = (res['cards'] as List).length;
          totalCards += count;
          _addLog('success', '  ✓ $fileName → $count cards imported');
        } catch (e) {
      ...
      setState(() {
        _running       = false;
        _lastRun       = _formatDateTime(today);
        _totalImported += files.length;
        _totalCards    += totalCards;
      });
      _addLog('success', '=== Run Complete: $totalCards cards from ${files.length} files ===');
    } catch (e) {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'Clear log',
            onPressed: () => setState(() => _scheduleLogs.clear()),
          ),
        ],
      ),
      body: Row(children: [
          ...
          child: SingleChildScrollView(
            ...
            child: Column(children: [
              ...
              _card('SFTP Connection', Icons.settings_ethernet, Column(children: [
                ...
                const SizedBox(height: 10),
                _field('Base Path (Emboss Directory)', _pathCtrl,
                    hint: '/cardzone/oneswitch_batch/log/batch_output'),
                const SizedBox(height: 4),
                Text('System ຈະ append /YYYYMMDD/emboss ອັດຕະໂນມັດ',
                ...
                Row(children: [
                  Expanded(child: ElevatedButton.icon(
                    ...
                    label: Text(_saved ? 'Saved ✓' : 'Save Config', style: const TextStyle(fontSize: 12)),
                  ...
                  Expanded(child: ElevatedButton.icon(
                    ...
                    label: Text(_connecting ? 'Testing...' : _connected ? 'Connected ✓' : 'Test Connection',
              ...
              _card('Schedule Settings', Icons.timer, Column(children: [
                ...
                Container(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(_scheduleEnabled ? 'Schedule Active' : 'Schedule Disabled',
                          style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600,
                              color: _scheduleEnabled ? const Color(0xFF1E7145) : Colors.grey.shade600)),
                      Text('Auto import emboss files ຕາມເວລາທີ່ກຳນົດ',
                          style: TextStyle(fontSize: 10, color: Colors.grey.shade500)),
                    ])),
                    Switch(
                      value: _scheduleEnabled,
                      onChanged: (v) { setState(() => _scheduleEnabled = v); _saveConfig(); },
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: const [
                        ...
                        DropdownMenuItem(value: 'manual',  child: Text('Manual Only', style: TextStyle(fontSize: 13))),
                      ],
                      onChanged: (v) => setState(() => _scheduleInterval = v!),
                    ),
                  ])),
                ...
                Container(
                  ...
                  child: Row(children: [
                    const Icon(Icons.auto_awesome, size: 18, color: Color(0xFF2E75B6)),
                    const SizedBox(width: 10),
                    const Expanded(child: Text('Auto import ທຸກ file ໃນ emboss',
                        style: TextStyle(fontSize: 12))),
                    Switch(
                      value: _autoImportAll,
                      onChanged: (v) => setState(() => _autoImportAll = v),
                      activeColor: _accent,
                    ),
                  ]),
                ),

                if (_scheduleEnabled && _scheduleInterval != 'manual') ...[
                  const SizedBox(height: 10),
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      const SizedBox(width: 8),
                      Expanded(child: Text(
                        _scheduleInterval == 'hourly'
                            ? 'ຈະ import ທຸກ 1 ຊົ່ວໂມງ ອັດຕະໂນມັດ'
                            : 'ຈະ import ທຸກວັນ ເວລາ $_scheduleTime',
              ...
              _card('Manual Run', Icons.play_arrow, Column(children: [
                Text('Import emboss files ຂອງວັນນີ້ ທັນທີ',
                    style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                const SizedBox(height: 4),
                Text(
                  'Path: ${_pathCtrl.text.trim()}/${_todayFolder()}/emboss',
                  style: const TextStyle(fontSize: 10, fontFamily: 'monospace', color: Color(0xFF185FA5)),
                ),
                const SizedBox(height: 12),
                SizedBox(width: double.infinity, child: ElevatedButton.icon(
                  ...
                  label: Text(_running ? 'ກຳລັງ import...' : 'Run Now (Today: ${_todayFolder()})',
          ...
          child: Column(children: [
            Container(
              ...
              child: Row(children: [
                ...
                const Spacer(),
                Text('${_scheduleLogs.length} entries',
                    style: const TextStyle(fontSize: 10, color: Color(0xFF666666))),
                const SizedBox(width: 8),
                GestureDetector(
                  onTap: () => setState(() => _scheduleLogs.clear()),
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemCount: _scheduleLogs.length,
                itemBuilder: (_, i) {
                  final log = _scheduleLogs[i];
                  return Padding(
